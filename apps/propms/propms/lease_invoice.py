...
@frappe.whitelist()
def leaseInvoiceAutoCreateNextMonth(data):
    """Prepare data to create sales invoice from lease invoice schedule up to the end of next month only."""
    try:
        ...
        generate_next_month_only = frappe.db.get_single_value(
            "Property Management Settings", "generate_invoice_schedule_next_month"
        )
        
        if not generate_next_month_only:
            return "Feature not enabled. Set 'Generate Invoice Schedule Only for Next Month' in Property Management Settings."
            
        # Get start date from data or use default
        invoice_start_date = data.get("start_date") if data and hasattr(data, "get") else None
        
        # Calculate end of next month based on invoice_start_date
        start_date = getdate(invoice_start_date)
        next_month_end = frappe.utils.get_last_day(
            frappe.utils.add_months(start_date, 1)
        )

        lease_invoice = frappe.get_all(
            "Lease Invoice Schedule",
            filters={
                "date_to_invoice": ["between", (invoice_start_date, next_month_end)],
                "invoice_number": "",
                "sales_order_number": ""
            },
            fields=[
